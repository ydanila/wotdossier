			...
			if 'arenaUniqueID' in myblock:
				...
				if 'vehicles' in br_block:
					for key, value in br_block['vehicles'].items():
						...
						if br_block['vehicles'][key][0]['typeCompDescr'] > 0:
							br_block['vehicles'][key][0]['tankID'] = br_block['vehicles'][key][0]['typeCompDescr'] >> 8 & 65535
							br_block['vehicles'][key][0]['countryID'] = br_block['vehicles'][key][0]['typeCompDescr'] >> 4 & 15
						
#						if 'details' in br_block['vehicles'][key][0]:
#							del br_block['vehicles'][key][0]['details']
...
#								break
	...
	if option_advanced==1 or option_chat==1:
		...
		uncompressed = filename_source + ".tmp.out"
...
def is_supported_replay(f):
	...
	if not versionlength in (8, 10, 11):
		return False
